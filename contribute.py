...
def main(def_args=sys.argv[1:]):
    ...
    # ✅ Extract directory name from repository URL safely
    if repository:
        repo_name = os.path.basename(urlparse(repository).path)
    ...
    # ✅ Validate directory
    if not directory:
        sys.exit("❌ Directory name could not be determined from the repository URL.")
...
def arguments(argsval):
    ...
    parser.add_argument('-mc', '--max_commits', type=int, default=10, required=False,
                        help="Maximum number of commits per day (1 to 20)")
    parser.add_argument('-fr', '--frequency', type=int, default=80, required=False,
                        help="Percentage of days to perform commits (0–100)")
    parser.add_argument('-r', '--repository', type=str, required=False,
                        help="Remote Git repository URL (SSH or HTTPS)")
    parser.add_argument('-un', '--user_name', type=str, required=False,
                        help="Override git user.name config")
    parser.add_argument('-ue', '--user_email', type=str, required=False,
                        help="Override git user.email config")
    parser.add_argument('-db', '--days_before', type=int, default=365, required=False,
                        help="Days before today to start making commits")
    parser.add_argument('-da', '--days_after', type=int, default=0, required=False,
                        help="Days after today to continue making commits")
    return parser.parse_args(argsval)
